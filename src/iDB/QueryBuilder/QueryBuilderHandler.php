...
class QueryBuilderHandler {
    ...
    /**
     * @var \iDB\QueryBuilder\Adapters\BaseAdapter
     */
    ...
    public function __construct(Connection $connection = null, $fetchMode = PDO::FETCH_OBJ) {
        ...
        $this->adapterInstance = $this->container->build(
            '\\iDB\\QueryBuilder\\Adapters\\'.ucfirst($this->adapter),
            [$this->connection]
        );
     ...
     * @return Raw
     */
    public function subQuery(self $queryBuilder, $alias = null) {
    ...
    /**
     * @param array|string $tables Single table or array of tables
     *
     * @throws Exception
     *
     * @return QueryBuilderHandler
    ...
    public function page($page) {
        if ($this->statements['limit']) {
            $this->statements['offset'] = ($page - 1) * $this->statements['limit'];
        }
    ...
    public function join($table, $key, $operator = null, $value = null, $type = 'inner') {
        if (!$key instanceof \Closure) {
            $key = function($joinBuilder) use ($key, $operator, $value): void {
        ...
        $joinBuilder = $this->container->build('\\iDB\\QueryBuilder\\JoinBuilder', [$this->connection]);
    ...
    public function transaction(\Closure $callback) {
        try {
            ...
            $transaction = $this->container->build('\\iDB\\QueryBuilder\\Transaction', [$this->connection]);
    ...
    public function raw($value, $bindings = []) {
        return $this->container->build('\\iDB\\QueryBuilder\\Raw', [$value, $bindings]);
    }
     ...
     * @param $value
     */
    protected function addStatement($key, $value): void {
        if (!\is_array($value)) {
            $value = [$value];
        }
